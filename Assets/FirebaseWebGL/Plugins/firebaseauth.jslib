mergeInto(LibraryManager.library, {
    ...
    SignInWithGoogle: function (objectName, callback, fallback) {
        ...
        try {
            ...
            firebase.auth().signInWithPopup(provider).then(function (unused) {			
                MyInstance.SendMessage(parsedObjectName, parsedCallback, "Success: signed in with Google!");
            }).catch(function (error) {
                MyInstance.SendMessage(parsedObjectName, parsedFallback, JSON.stringify(error, Object.getOwnPropertyNames(error)));
            });

        } catch (error) {
            MyInstance.Module.SendMessage(parsedObjectName, parsedFallback, JSON.stringify(error, Object.getOwnPropertyNames(error)));
        }
    },
    ...
    SignInWithFacebook: function (objectName, callback, fallback) {
        ...
        try {
            ...
            firebase.auth().signInWithPopup(provider).then(function (unused) {
                MyInstance.Module.SendMessage(parsedObjectName, parsedCallback, "Success: signed in with Facebook!");
            }).catch(function (error) {
                MyInstance.Module.SendMessage(parsedObjectName, parsedFallback, JSON.stringify(error, Object.getOwnPropertyNames(error)));
            });
        ...
        try {
            ...
            firebase.auth().signInWithPopup(provider).then(function (unused) {
                MyInstance.Module.SendMessage(parsedObjectName, parsedCallback, "Success: signed in with Microsoft!");
            }).catch(function (error) {
                MyInstance.Module.SendMessage(parsedObjectName, parsedFallback, JSON.stringify(error, Object.getOwnPropertyNames(error)));
            });
    ...
    OnAuthStateChanged: function (objectName, onUserSignedIn, onUserSignedOut) {
        ...
        firebase.auth().onAuthStateChanged(function(user) {
            if (user) {
                MyInstance.Module.SendMessage(parsedObjectName, parsedOnUserSignedIn, JSON.stringify(user));
            } else {
                MyInstance.Module.SendMessage(parsedObjectName, parsedOnUserSignedOut, "User signed out");
            }
        });
